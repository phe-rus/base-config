...
```
packages/ui/src/
  components/         vendored shadcn/base-ui primitives: badge, breadcrumb, button, calendar, checkbox, collapsible, combobox, dialog, drawer, field, input, input-group, label, popover, radio-group, select, separator, skeleton, sonner, switch, table, tabs, textarea. `sonner.tsx` wraps `goey-toast` (a real dependency, not a vendored shadcn primitive). `code-editor.tsx` is the odd one out, not a vendored shadcn primitive but a real, form-agnostic CodeMirror 6 wrapper (`CodeEditor`, `@uiw/react-codemirror` + `@codemirror/{lang-javascript,lang-json,view}`), exported as `@baseconfig/ui/components/code-editor` for reuse outside `@baseconfig/ui/forms` too (`@baseconfig/core`'s `code` block uses it directly for its own corner language toggle). See `src/forms/CLAUDE.md` for the `Code`/`JSON` fields built on top of it.
