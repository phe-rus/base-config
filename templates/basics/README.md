...
A minimal, working reference app for [`@baseconfig/core`](https://www.npmjs.com/package/@baseconfig/core): one `posts` collection, one `settings` global, email+password admin auth, deployed as a single Cloudflare Worker. No plugins, no extra sign-in methods — just enough to see the whole stack working end to end.
...
Visit `/admin` and create an account — the **first** account created becomes an admin automatically. From there you can create/edit `posts` and the `settings` global.
...
- `src/config/base.config.ts` — the whole content model in one call: one collection (`posts`), one global (`settings`).
