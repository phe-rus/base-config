...
This repo (`base-config`) is published to npm as four packages (`@baseconfig/core`, `@baseconfig/ui`, `@baseconfig/cli`, `@baseconfig/plugin-form-builder`, see Publishing below). `www` is a real consumer app living in this same monorepo (`www/`, a Bun workspace member, `workspace:*` on `core`/`ui`), not a separate repo; `templates/basics`/`templates/pharmacy` are consumer apps too, but pin published versions instead of a workspace link. See Workspace layout below for what's runnable/editable where.
...
- **`templates/basics`** is the reference/dev-testbed app: `users`/`posts` collections, `keywords`/`storage` globals, email+password-only auth, no plugins, the minimal end-to-end wiring example, and the single source of truth `@baseconfig/cli` bundles as its own scaffold output. Pins `@baseconfig/core`/`@baseconfig/ui` to the published `0.0.14` (older than the current workspace build), so `bun x base` there runs the registry-cached core, not this repo's `packages/config` source.
- **`templates/pharmacy`** is a fuller, e-commerce-style reference app: `users`/`products`/`posts` collections, `keywords`/`storage` globals, same minimal auth as `basics`. Pins `@baseconfig/core`/`@baseconfig/ui` to the published `0.0.16`, same reasoning as `basics`. **Pin it to a version genuinely behind the current workspace build, never the exact current `packages/config`/`packages/ui` version**: Bun auto-links a workspace member's dependency to the local package whenever the declared version string matches that local package's own `version` field, even a plain semver string, not just `workspace:*`, confirmed the hard way (pinning to the then-current `0.0.17` still silently symlinked straight to `packages/config`/`packages/ui` source, `bun install --force` and a full `node_modules` wipe included, until repinned to the previous published version).
